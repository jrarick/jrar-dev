...
export default function Intro() {
  ...
  const paragraphsText = [
    ...
    'Diagnosing and fixing tricky browser performance issues',
    ...
    'And even more, but the real question is:',
  ...
  return (
    <div className="pb-[36rem] pt-16">
      <div className="font-medium px-3 sm:px-12 space-y-16" ref={IntroContainerRef}>
        {paragraphsText.map((text, i) => (
          <p
            key={text}
            id={`intro-p-${i + 1}`}
            className="text-base h-4 sm:text-xl sm:h-5 md:text-2xl/6 md:h-6"
          />
        ))}
      </div>
    </div>

...
export function BookmarkTree({ items }: BookmarkTreeProps) {
  return (
    <div className="font-mono bg-app-background p-2" role="tree" aria-label="Bookmarks">
      {items.map((item) => (
...
function TreeItem({ item, level }: TreeItemProps) {
  ...
  return (
    <div role="treeitem" aria-expanded={item.type === "folder" ? isExpanded : undefined}>
      <div
        className="group flex items-center gap-2 py-1.5 px-2 cursor-default select-none text-sm hover:bg-matrix-background/50 hover:text-matrix-vivid"
      ...
      >
        ...
        {/* Icon */}
        {item.type === "folder" ? (
          <Folder className="w-4 h-4 text-matrix-base flex-shrink-0" />
        ) : item.favicon_url ? (
          <img
        ...
        {item.type === "bookmark" && item.url ? (
          <a
            ...
            className="flex-1 truncate text-app-foreground hover:text-matrix-vivid hover:underline"

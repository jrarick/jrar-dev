...
- **Links**: ALWAYS use the `Link` component from `~/components/link` for internal links. Do NOT use `Link` or `NavLink` from `react-router` directly in UI components. The `Router` provider is already configured to handle client-side routing for `react-aria-components`. Active states should be determined using `useLocation`.
